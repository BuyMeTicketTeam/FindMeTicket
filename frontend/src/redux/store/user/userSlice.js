...
function setUser(state, action) {
  ...
  state.userPhoto =
    action.payload.socialMediaAvatar ??
    `data:image/jpeg;base64,${action.payload.defaultAvatar}`;
}
...
const slice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addMatcher(
      userApi.endpoints.notificationEnable.matchFulfilled,
      (state) => {
        state.notification = true;
      }
    );
    builder.addMatcher(
      userApi.endpoints.notificationDisable.matchFulfilled,
      (state) => {
