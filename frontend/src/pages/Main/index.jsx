...
export default function Index() {
  ...
  const {
    departureCity, arrivalCity, departureDate, bus,
    train, airplane, ferry, sortBy, ascending,
  ...
  useEffect(() => {
    ...
    if (tickets.length === 0) {
      searchTickets({ data: payload, onChunk: (value) => setTickets((prevData) => [...prevData, value]) });
    } else {
      sortTickets(payload);
    }
  }, [departureCity, arrivalCity, departureDate, bus, train, airplane, ferry, sortBy, ascending, searchTickets, sortTickets]);
      ...
      <div className="container">
        ...
        <>
          ...
          <div className="tickets">
            {tickets.map((item) => <Ticket key={item.data.id} data={item.data} />)}
          </div>
        </>
        )}
