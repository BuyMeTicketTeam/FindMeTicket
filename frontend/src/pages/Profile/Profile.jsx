...
export default function Profile() {
  ...
  const { username, userPhoto, userEmail, notification } = useSelector(
    (state) => state.user
  );
  ...
  const [enableNotification, { isError: enableNotificationError }] =
    useLazyNotificationEnableQuery();
  const [disableNotification, { isError: disableNotificationError }] =
    useLazyNotificationDisableQuery();
      ...
      <div className="container type_profile">
        <div className="profile-error-container">
          {enableNotificationError && (
            <p className="error">{t("error_enable_notification")}</p>
          )}
          {disableNotificationError && (
            <p className="error">{t("error_disable_notification")}</p>
          )}
        ...
        <div className="profile-block gen-info">
            ...
            <Popup
              ...
              trigger={
                <img
                  src={mark}
                  alt="mark"
                  className="gen-info-level__tip-img"
                />
              }
          ...
          >
            <img src={logoutImg} alt="Exit" className="gen-info__exit-icon" />
          </button>
        </div>
        <div className="profile-block profile-notification">
          <p className="profile-notification__text">{t("notification")}</p>
          ...
          <div className="profile-contacts-group">
            <div className="profile-contacts-group__column">
              <div className="profile-contacts-group__row">
                <img
                  src={emailIcon}
                  className="profile-contacts-group__icon"
                  alt="Email"
                />
                <p className="profile-contacts-group__text">{t("email")}</p>
              </div>
              <div className="profile-contacts-group__row">{userEmail}</div>
            </div>
            <div className="profile-contacts-group__column">
              <div className="profile-contacts-group__row content_center">
                <img
                  src={phoneIcon}
                  className="profile-contacts-group__icon type_phone"
                  alt="Phone"
                />
        ...
        <button
          className={`profile-block profile-history-btn ${
        ...
        >
          <img
            src={historyIcon}
            alt="History"
            className="profile-history-btn__icon"
          />
          <p className="profile-history-btn__text">{t("history")}</p>
          <div
            className={`profile-history-btn__arrow ${
              isHistoryExpanded ? "arrow-down" : ""
            }`}
          />
        </button>
        {isHistoryExpanded && (
          <div className="profile-history">
            {isLoading && (
              <img className="loading" src={loaderIcon} alt="Loading..." />
            )}
            {!isLoading && history && history.length > 0 ? (
              history.map((historyItem) => (
                <Link to="/" key={uuidv4()} className="profile-history-item">
                  <span className="profile-history-item__search-date">
                    {historyItem.addingTime}
                  </span>
                  <img
                    src={getIcon(historyItem)}
                    alt="Transport icon"
                    className="profile-history-item__icon"
                  />
                  <span className="profile-history-item__from">
                    {historyItem.departureCity}
                  </span>
                  <div className="profile-history-item__column">
                    <div className="profile-history-item__date">
                      {historyItem.departureDate}
                    </div>
                    <hr className="profile-history-item__line" />
                  </div>
                  <span className="profile-history-item__to">
                    {historyItem.arrivalCity}
                  </span>
                </Link>
              ))
            ) : (
              <div className="profile-history-item no-items">
                {t("no_items")}
              </div>
            )}
          </div>
