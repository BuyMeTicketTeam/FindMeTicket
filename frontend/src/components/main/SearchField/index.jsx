...
export default function SearchField({
  ...
  function handleRequest() {
    ...
    navigate(
      `?type=${searchParams.get("type")}&from=${cityFrom.value}&to=${
        cityTo.value
      }&departureDate=${+date}&endpoint=1`
    );
  }
  ...
  useEffect(() => {
    ...
    if (params && location.pathname !== "/login") {
      navigate(`?${location.search.replace("?", "")}${params}`, {
        replace: true,
      });
    }
    const body = {
    ...
    setCityFrom(
      body.departureCity
        ? { value: body.departureCity, label: body.departureCity }
        : ""
    );
    setCityTo(
      body.arrivalCity
        ? { value: body.arrivalCity, label: body.arrivalCity }
        : ""
    );
