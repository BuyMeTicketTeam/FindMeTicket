...
function Popup({ setIsProfilePopup, updateAuthValue, status }) {
  const [avatarImage, setAvatarImage] = useState(defaultAvatarUrl);
  ...
  return (
    <div className="popup open">
      <div className="popup-content">
        ...
        />
        <div className="custom-input">{status.nickname}</div>
        <p>
          <Link to="/change-password" className="change-password" onClick={() => setIsProfilePopup(false)}>Змінити пароль</Link>
        </p>
        <button type="button" className="exit" onClick={() => { setIsProfilePopup(false); handleLogoutButton(); }}>Вийти</button>

        <button type="button" className="privacy-policy-link" onClick={handleDeleteAccount}>
          <a href="https://www.freeprivacypolicy.com/live/de2f7052-4f8d-42a4-ab02-0595dc2b02ac" target="_blank" rel="noopener noreferrer">
