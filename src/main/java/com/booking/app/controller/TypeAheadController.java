...
public class TypeAheadController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400"
                    , description = "Required header ContentLanguage is not present. " +
                    "OR" + " Required parameter 'startLetters' is not present.",
    ...
    public ResponseEntity<List<CityDto>> getCities(@RequestParam("startLetters") String startLetters,
                                                   @RequestHeader(name = HttpHeaders.CONTENT_LANGUAGE) String contentLanguage) throws

            IOException {
        return ResponseEntity.ok().body(typeAheadService.findMatches(startLetters, contentLanguage));
    }
