...
import com.booking.app.utils.SseEmitterUtils;
...
public class InfobusBusServiceImpl implements ScraperService {
    ...
    private static void processTicketInfo(SseEmitter emitter, Ticket ticket, List<WebElement> elements, WebDriver driver, WebDriverWait wait, MutableBoolean emitterNotExpired) throws IOException {
        BusTicket busTicket = (BusTicket) ticket;
        BusInfo priceInfo = busTicket.getInfoList().stream().filter(t -> t.getWebsite().equals(Website.INFOBUS)).findFirst().get();
...
//                wait.until(ExpectedConditions.urlContains("deeplink"));
    ...
    private static BusTicket scrapeTicketInfo(WebElement webTicket, Route route) throws ParseException {
        ...
        return BusTicket.createInstance(route,
                webTicket.findElement(By.cssSelector("div.departure")).findElement(By.cssSelector("a.text-g")).getText(),
                webTicket.findElement(By.cssSelector("div.arrival")).findElement(By.cssSelector("a.text-g")).getText(),
                LocalTime.parse(webTicket.findElement(By.cssSelector("div.departure")).findElement(By.cssSelector("div.day_time")).findElements(By.tagName("span")).get(2).getText()),
                Instant.parse(webTicket.findElement(By.cssSelector("div.arrival")).findElement(By.cssSelector("div.day_time")).findElements(By.tagName("span")).get(2).getText()),
...
//                emitter.send(event.name("Infobus bus: ").data(busMapper.ticketToTicketDto(scrapedTicket, language)));
            } else {
